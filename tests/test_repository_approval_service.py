...
class CLIShimTests(unittest.TestCase):
    """``scripts/approve_repository.py`` should defer to the service."""
    ...
    def setUp(self) -> None:
        ...
        self._env_patch = patch.dict(
            os.environ,
            {APPROVED_REPOSITORIES_PATH_ENV_KEY: str(self.tmpdir / 'sidecar.json')},
            clear=False,
        )
        self._env_patch.start()
        self.addCleanup(self._env_patch.stop)

    def test_approve_subcommand_writes_sidecar_entry(self) -> None:
        from scripts import approve_repository

        rc = approve_repository.main(
            ['approve', 'vendor-sdk', '--remote', 'git@example.com:v.git'],
        )

        self.assertEqual(rc, 0)
        service = RepositoryApprovalService(
            storage_path=self.tmpdir / 'sidecar.json',
        )
        self.assertEqual(service.is_approved('vendor-sdk'), ApprovalMode.RESTRICTED)

    def test_approve_with_trusted_flag_records_trusted_mode(self) -> None:
        from scripts import approve_repository

        rc = approve_repository.main(
            ['approve', 'vendor-sdk', '--remote', 'git@example.com:v.git', '--trusted'],
        )
        ...
        self.assertEqual(service.is_approved('vendor-sdk'), ApprovalMode.TRUSTED)

    def test_revoke_subcommand_returns_zero_when_entry_existed(self) -> None:
        from scripts import approve_repository

        approve_repository.main(
            ['approve', 'vendor-sdk', '--remote', 'git@example.com:v.git'],
        )
        rc = approve_repository.main(['revoke', 'vendor-sdk'])

        self.assertEqual(rc, 0)

    def test_revoke_subcommand_returns_nonzero_when_no_entry(self) -> None:
        from scripts import approve_repository

        rc = approve_repository.main(['revoke', 'never-approved'])
