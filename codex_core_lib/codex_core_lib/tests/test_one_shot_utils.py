"""Tests for ``one_shot`` / ``make_one_shot``.
...
class CodexOneShotTests(unittest.TestCase):
    ...
    def test_oserror_raises_codex_one_shot_error(self) -> None:
        with patch(
            ...
            with self.assertRaises(OneShotError) as ctx:
                one_shot('hi')
...
class OneShotFileReadEdgeCases(unittest.TestCase):
    ...
    the ``--output-last-message`` file in ``one_shot``."""
    ...
    def test_unlink_oserror_in_finally_is_swallowed(self) -> None:
        ...
        # helper's contract is "return the text or raise OneShotError",
...
class MakeCodexOneShotTests(unittest.TestCase):
    def test_closure_passes_fixed_config(self) -> None:
        call = make_one_shot(binary='/opt/codex', model='codex-x', timeout_seconds=30)
        with patch(
            'agent_core_lib.agent_core_lib.helpers.one_shot.subprocess.run',
