...
class BitbucketClient(PullRequestClientBase):
    ...
    def list_pull_request_comments(
        ...
        while url:
            ...
            # Bitbucket's ``next`` is a FULLY QUALIFIED URL. Pass it straight
            # through: every verb builds its URL via ``RetryingClientBase._abs_url``,
            # which leaves an already-absolute URL untouched, so this fetches the
            # real next page instead of the doubled, 404-ing ``base_url + next``
            # that used to silently drop every comment past page 1. No
            # prefix-stripping dance needed now that the guard lives in the one
            # place every verb goes through.
