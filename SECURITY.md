...
| Operator enables `KATO_CLAUDE_BYPASS_PERMISSIONS` without realizing | Flag ships `false` in `.env.example`. Defense-in-depth: (a) **refused outright when the kato process runs as root**, (b) **requires `KATO_CLAUDE_BYPASS_PERMISSIONS_ACCEPT=true`** for non-interactive runs (CI/Docker/cron) — kato refuses to start otherwise, (c) **interactive y/n confirmation prompt** at startup when stdin is a TTY and the ACCEPT flag is not set, (d) **unmissable stderr banner** at every kato boot when the flag is true, written before logger configuration so log level cannot suppress it, (e) **persistent red banner across the top of the planning UI** so the bypass state is visible to every operator looking at the browser, (f) the configurator (`python -m kato.configure_project`) requires typing the literal phrase `I ACCEPT` before writing the flag, (g) `WARNING` log on every Claude spawn naming the operator's responsibility, (h) README "Operator responsibilities" section. | **Strong.** Cannot prevent a deliberate operator decision but makes silent or accidental enablement effectively impossible. |
