...
> **Bypass mode operators:** the concrete protections that apply when `KATO_CLAUDE_BYPASS_PERMISSIONS=true` (Docker sandbox, egress firewall, capability drop, two-key startup gate) are documented in [BYPASS_PROTECTIONS.md](BYPASS_PROTECTIONS.md). Read both — this file is the threat model, that file is the countermeasures.
...
| Prompt-injection via task content | Prompt-level guardrails ([cli_client.py](kato_core_lib/client/claude/cli_client.py)) ask the agent not to follow embedded instructions, plus the per-tool permission layer above. | **Weak / advisory.** A determined injection can still steer the agent; the permission layer is what actually catches it. |
| Operator enables `KATO_CLAUDE_BYPASS_PERMISSIONS` without realizing | Flag ships `false` in `.env.example`. Defense-in-depth: (a) **refused outright when the kato process runs as root**, (b) **refused outright under non-interactive runs** (CI/Docker/cron/systemd — no flag-only escape hatch; acknowledgement must come from a real TTY), (c) **double y/n confirmation prompt** at startup when stdin is a TTY (uses `prompt_yes_no` so a fat-fingered Enter cannot select default), (d) **unmissable stderr banner** at every kato boot when the flag is true, written before logger configuration so log level cannot suppress it, (e) **persistent red banner across the top of the planning UI** so the bypass state is visible to every operator looking at the browser, (f) the configurator (`python -m kato_core_lib.configure_project`) requires typing the literal phrase `I ACCEPT` before writing the flag, (g) `WARNING` log on every Claude spawn naming the operator's responsibility, (h) README "Operator responsibilities" section, (i) hardened Docker sandbox bounds the consequences if it does run — see [BYPASS_PROTECTIONS.md](BYPASS_PROTECTIONS.md). | **Strong.** Cannot prevent a deliberate operator decision but makes silent or accidental enablement effectively impossible. |
