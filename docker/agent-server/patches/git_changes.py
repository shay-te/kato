...
def get_git_changes(cwd: str | Path) -> list[GitChange]:
    ...
    # Add changes from git directories, skipping any that are inaccessible
    # (e.g. Windows volume mounts where .git exists but git refuses to operate)
    for git_dir in git_dirs:
        try:
            git_dir_changes = get_changes_in_repo(str(Path(cwd, git_dir)))
        except (GitRepositoryError, GitCommandError) as e:
            logger.warning(f"Skipping {git_dir}: {e}")
            continue
