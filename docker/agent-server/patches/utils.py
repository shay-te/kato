...
def validate_git_repository(repo_dir: str | Path) -> Path:
    ...
    # Always verify git recognises the directory as a repository.
    # Checking for .git existence is not enough — on Windows volume mounts the
    # .git file/directory can exist but git may still refuse to operate on it.
    try:
