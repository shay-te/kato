...
| `KATO_TASK_COMMENTS_ENABLED` | Whether comments on the ticket (YouTrack/Jira/GitHub/GitLab/Bitbucket issues) are folded into the task the agent works from. **Off by default** — the agent sees only the ticket description. Set to `true` to opt in. |
| `KATO_TASK_COMMENTS_REQUIRE_MENTION` | Only meaningful when the above is on. `true` (default) takes a ticket comment ONLY when it `@mentions` the kato user — a comment tagging nobody, or tagging someone else, never becomes an instruction. `false` restores the older, looser rule (take everything except comments tagging other people). Kato's mention identity is the configured `assignee`; when that is unset or an alias (`me`, `currentUser()`) it is resolved from the platform's current-user endpoint, and if it cannot be resolved the strict rule takes **no** comments rather than guessing. |
| `KATO_REVIEW_COMMENTS_ENABLED` | Master switch for the **pull-request review-comment** loop (Bitbucket/GitHub/GitLab). **On by default.** Set to `false` and kato makes no comment-polling call to the git host at all, refuses to start a review-fix batch that was already queued, and — when flipped from the Settings UI — terminates the review-comment run already in flight (the same teardown as the Stop button, so no workspace restore and no retry). Read fresh from `~/.kato/settings.json` on every check, so it applies **immediately, without a restart**. Does not affect ticket pickup, ticket comments (`KATO_TASK_COMMENTS_ENABLED`), or the diff comments you write in kato's own Files tab. |
| `KATO_REVIEW_COMMENTS_REQUIRE_MENTION` | **On by default.** Kato acts on a pull-request review comment ONLY when it `@mentions` kato's own user, and ignores every other comment on the PR — including ones that tag nobody, which are reviewers talking to each other. Set to `false` for the older, looser rule (act on everything except comments tagging someone else). Kato's mention identity is its code-host login (`BITBUCKET_USERNAME` / `GITHUB_ASSIGNEE` / …) plus the ticket-platform assignee — so you tag *that* account, which is why a dedicated kato user is worth setting up (see readmeIssuePlatforms.md). **Fails closed:** if that identity can't be resolved, no mention can match and kato answers nothing. Read fresh on every check — applies immediately, no restart. |
...
| `KATO_SANDBOX_AUDIT_REQUIRED` | Refuse the spawn when the sandbox audit log cannot be written, instead of proceeding unaudited. |
| `AGENT_SANDBOX_MAX_CONTAINER_SECONDS` | Ceiling on how long ONE sandbox container may live. Default `8h`; `0` disables. A wedged agent otherwise holds its workspace mount and credentials open indefinitely, and nothing notices because nothing died. |
| `AGENT_SANDBOX_EGRESS_PROXY_IP` | Set by kato INTO the container — not something an operator configures. Carries the address of that task's SNI-pinning egress proxy: the in-container firewall then permits TCP/443 to that address only, and disables DNS entirely (with egress proxy-pinned there is nothing left to resolve). |
