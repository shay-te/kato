...
| 3 | Hard requirement for Docker daemon | [`sandbox/manager.py`](sandbox_core_lib/sandbox_core_lib/manager.py), gated in [`main.py`](kato_core_lib/main.py) |
| 4 | Filesystem boundary (per-task workspace is the only host bind-mount) | [`sandbox/manager.py:wrap_command`](sandbox_core_lib/sandbox_core_lib/manager.py) |
| 5 | Default-DROP egress firewall (allowlist = `api.anthropic.com` only) | [`sandbox/init-firewall.sh`](sandbox_core_lib/sandbox_core_lib/init-firewall.sh) |
| 6 | Capability drop + non-root user inside the container | [`sandbox/entrypoint.sh`](sandbox_core_lib/sandbox_core_lib/entrypoint.sh), [`Dockerfile`](sandbox_core_lib/sandbox_core_lib/Dockerfile) |
...
| L4 filesystem boundary + workspace validator | Active (Linux paths) | Active | Active | **Active + macOS-specific subtrees** in the forbidden list ([`manager.py:181-218`](sandbox_core_lib/sandbox_core_lib/manager.py)) | Active (WSL2 paths look like Linux) | **Refused** | Unsupported |
...
- That AppArmor would catch a container escape on Linux. AppArmor is one belt; cap-drop, read-only rootfs and gVisor (always — the spawn is refused without it) are the others.

Operators on Windows: install [Docker Desktop with WSL2 backend](https://docs.docker.com/desktop/wsl/), then run kato from inside a WSL2 distribution (Ubuntu, Debian). Note that gVisor is **mandatory** and Docker Desktop cannot register the `runsc` runtime — there is no override — so docker mode needs a WSL2 distro (or VM) where you can install gVisor and register it with the daemon yourself. Every other layer applies as it would on Linux native.
...
- **Module ready, integration pending** — a module under
  `sandbox_core_lib.sandbox_core_lib.*` (or equivalent) implements the
...
| OG2 | **External audit-log shipping** — ship the hash-chain audit log to S3-with-Object-Lock or a syslog forwarder so tail-truncation is detectable. | Closes the "tail-truncation looks valid" residual the doc names under "What the hash chain proves (and doesn't)." | **Closed** — `sandbox_core_lib.sandbox_core_lib.audit_log_shipping`. Operator sets `KATO_SANDBOX_AUDIT_SHIP_TARGET` to `https://...` or `file://...`; each spawn's audit entry is shipped after the local write. Best-effort by default; `KATO_SANDBOX_AUDIT_SHIP_REQUIRED=true` fails-closed. Cross-OS (stdlib only). |
...
| OG9a | **Workspace-content delimiter framing** — wrap workspace file content in `<UNTRUSTED_WORKSPACE_FILE>...</UNTRUSTED_WORKSPACE_FILE>` markers; instruct the model to treat content inside the markers as data, not instructions. | Closes the most-naive prompt-injection class on workspace content (READMEs / comments / fixtures that try to address the model directly). | **Closed** — `sandbox_core_lib.sandbox_core_lib.workspace_delimiter`. `wrap_untrusted_workspace_content(content, source_path=...)` returns the framed string, with literal close-tag forgery escaped. The system-prompt addendum (Section 5) instructs the model to treat in-tag content as data. Cross-OS (pure Python). |
...
The flags applied in [`wrap_command`](sandbox_core_lib/sandbox_core_lib/manager.py), the
firewall allowlist in [`init-firewall.sh`](sandbox_core_lib/sandbox_core_lib/init-firewall.sh),
the auth-volume mount semantics in [`entrypoint.sh`](sandbox_core_lib/sandbox_core_lib/entrypoint.sh),
...
5. **Persisting `settings.json`, `hooks/`, MCP config, `commands/`, or
   ...
   bidirectional manifest check in [`entrypoint.sh`](sandbox_core_lib/sandbox_core_lib/entrypoint.sh)
...
Inside the container, [`init-firewall.sh`](sandbox_core_lib/sandbox_core_lib/init-firewall.sh)
...
constants of the same name in [`sandbox_core_lib/sandbox_core_lib/manager.py`](sandbox_core_lib/sandbox_core_lib/manager.py)
...
Mechanical enforcement lives in [`entrypoint.sh`](sandbox_core_lib/sandbox_core_lib/entrypoint.sh),
[`wrap_command`](sandbox_core_lib/sandbox_core_lib/manager.py),
[`login_command`](sandbox_core_lib/sandbox_core_lib/manager.py), and the
...
Named tags for properties of [`init-firewall.sh`](sandbox_core_lib/sandbox_core_lib/init-firewall.sh)
