...
def workspace_scope_block(allowed_paths, extra_refusal_guidance: str = '') -> str:
    ...
    block = (
        ...
        'anything you want to keep between turns.\n\n'
        # A prohibition with no destination loses to habit: told only "not
        # /tmp", an agent that needs somewhere to put a throwaway script
        # still reaches for /tmp, because nothing else was offered. Name the
        # place instead. It sits at the task folder, ALONGSIDE the
        # repositories rather than inside one, so nothing there can end up
        # in a commit.
        'YOUR HELPER-SCRIPT DIRECTORY IS: '
        f'{primary}{os.sep}helper_scripts{os.sep}\n\n'
        'EVERYTHING you create that is not part of the deliverable goes '
        'there. Measured from real sessions, that means at least:\n'
        '  - one-off scripts, migration helpers, checks — anything you run '
        'once and throw away;\n'
        '  - whatever those scripts produce: test databases, dumps, '
        'backups, generated fixtures, downloaded files;\n'
        '  - your OWN notes: a plan, a scratch summary, memory you want on '
        'the next turn. Not the home directory, not an agent config '
        'directory somewhere else on the machine — here.\n'
        'Create it if it does not exist yet (``mkdir -p``). It is outside '
        'the repositories, so nothing you leave in it is ever committed.\n\n'
        'Do NOT delete anything from it — not the scripts, not their '
        'output. A helper you wrote three turns ago is very often the exact '
        'thing you need again two turns later, and rewriting it costs far '
        'more than the disk it was sitting on. Keep them. The whole '
        'directory is removed when the operator removes this task\'s '
        'workspace, which is the only cleanup anyone needs. Do not spend a '
        'turn tidying, and do not apologise for what is left there — '
        'leaving it is the intended behaviour.\n\n'
        'Do NOT use ``/tmp``, ``/var/tmp``, ``$TMPDIR``, the home '
        'directory, or anywhere else on the machine for this — not for a '
        '"quick" script, not for a single file, not even for something you '
        ...
        'and build managers with their own caches, ``/dev/null`` and the '
        'usual shell plumbing. You do not need permission for those and '
        'they are not what this rule is about. What you must not do is READ '
        'OR WRITE PROJECT FILES outside the paths below — another '
        'checkout of the same repository, a sibling project, the '
        'operator\'s own source tree.\n\n'
        'If the code you need lives in a repository that is NOT listed '
        'below, do not go and read it from wherever else it exists on this '
        'machine — that copy is not the one this task is working on, and '
        'editing it corrupts the operator\'s own tree. Ask for the '
        'repository to be added to this task, and wait.\n\n'
